...
const isTrueFlag = (v: unknown): boolean =>
...
function makeApi(
    ...
    }: { baseUrl: string; getAuth?: () => string | null; withCredentials?: boolean }
): ApiFn {
    return async function api<T = unknown>(path: string, opts: RequestInit = {}): Promise<T> {
        ...
        if (!r.ok) {
            ...
            if (ct.includes("application/json")) {
                ...
                if (isRecord(parsed)) msg = pickErrorMessage(parsed) ?? msg;
            } else {
...
function useOrgTreeData(api: ApiFn) {
    ...
    const loadChildren = React.useCallback(
        async (parentId: number) => {
            const url = `/${parentId}/children?showDeleted=true`;
    ...
    const ensureRoot = React.useCallback(async () => {
        if (rootId != null) {
            const cached = nodesRef.current.get(rootId);
            if (cached) return cached;
        }
        const root = await api<OrgNode>(`/root?showDeleted=true`);
        setNode(root);
        setRootId(root.id);
        return root;
    }, [api, setNode, rootId]);
    ...
    const expandPath = React.useCallback(
        async (ids: number[]) => {
            for (const id of ids) {
                await loadChildren(id);
    ...
    const move = React.useCallback(
        async (nodeId: number, newParentId: number) => {
            ...
            await api(`/${nodeId}/move`, {
                method: "PATCH",
                body: JSON.stringify({ newParentId, expectedVersion }),
            });
            ...
            const fresh = await api<OrgNode>(`/${nodeId}?showDeleted=true`);
            setNode(fresh);
            return fresh;
        },
        [api, setNode]
    ...
    const createChild = React.useCallback(
        async (
            parentId: number,
            payload: Partial<Pick<OrgNode, "code" | "name" | "isActive">> & {
                sortOrder?: number | null;
            }
        ) => {
            return await api<OrgNode>("", {
                method: "POST",
    ...
    const rename = React.useCallback(
        async (
            ...
            const expectedVersion = node?.version ?? null;
            const res = await api<OrgNode>(`/${id}`, {
                method: "PATCH",
...
type TreeNodeProps = {
    ...
    onCreate?: (
        parentId: number,
        payload: { code: string; name: string; isActive: boolean; sortOrder?: number | null }
    ) => Promise<OrgNode>;
    onRename?: (
        id: number,
        payload: Partial<Pick<OrgNode, "name" | "code" | "sortOrder" | "isActive">>
...
function TreeNode({
    ...
    const baseInputCn =
        "w-full border border-gray-200 rounded-md px-2.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500";
    const btnIconCn =
        "inline-flex items-center justify-center w-[22px] h-[22px] border border-gray-200 rounded bg-gray-50 hover:bg-gray-100 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed";
    ...
    const isDeleted = isTrueFlag(node.deleted);
    const inactiveCn =
        !node.isActive && !isDeleted ? "text-gray-400 line-through" : "text-gray-800";
    const deletedTextCn = isDeleted ? "!text-red-600 font-semibold" : "";
    const deletedBgCn = isDeleted
    ...
    const visibleKids = kids.filter((cid) => {
        ...
        const shouldHideInactive = !showInactive && !childNode.isActive;
        const shouldHideDeleted = !showDeleted && isTrueFlag(childNode.deleted);
        return !shouldHideInactive && !shouldHideDeleted;
    });
    ...
    return (
        <div>
            ...
            >
                ...
                {!editing ? (
                    <button
                        className="bg-transparent border-none cursor-pointer text-left flex-grow"
                        onClick={() => onSelect?.(node)}
                        title={node.path}
                    >
            <span
                className={`${tagCn} ${inactiveCn} ${deletedTextCn} ${
                    isDeleted ? "bg-red-100 text-red-700" : ""
                }`}
            >
              {node.code}
            </span>
                        <strong className={`font-semibold ml-1 ${inactiveCn} ${deletedTextCn}`}>
                            {node.name}
                        </strong>
                    </button>
                ) : (
                    ...
                    >
                        <input
                            ...
                            className={`${baseInputCn} p-1 w-64`}
                        />
                        <button type="submit" className={btnIconCn}>
                            ✔
                        </button>
                        <button
                            type="button"
                ...
                <div className="ml-auto flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                        className={`${btnIconCn} hover:bg-red-100`}
                        title="O'chirish"
                    ...
                    <button
                        className={btnIconCn}
                        title={node.isActive ? "Deaktivatsiya qilish" : "Aktivlashtirish"}
                        onClick={() => onRename?.(id, { isActive: !node.isActive })}
                        disabled={isDeleted}
                    >
                        👁️
                    </button>
            ...
            {adding && (
                <div
                    className="flex items-center gap-2 my-0.5 rounded-md bg-slate-50"
                    style={{
                ...
                >
                    ...
                    >
                        <input
                            ...
                            placeholder="Kod"
                            className={`${baseInputCn} p-1 w-28`}
                            required
                        />
                        <input
                            value={newName}
                            ...
                            placeholder="Nom"
                            className={`${baseInputCn} p-1 flex-grow`}
                            required
                        />
                        ...
                        <button type="submit" className={btnIconCn} title="Saqlash">
                            ✔
                        </button>
                        <button
                            type="button"
            ...
            {confirmingDelete && (
                <div
                    className="flex items-center justify-end gap-2 my-0.5 rounded-md bg-red-50 border border-red-200 text-xs p-2"
                ...
                >
                    ...
                    <button
                        ...
                        className="px-2 py-1 bg-yellow-400 text-yellow-900 rounded hover:bg-yellow-500"
                    >
                        Vaqtincha
                    </button>
                    <button
...
export default function OrgTree({
    ...
    const api = React.useMemo(
        () => makeApi({ baseUrl, getAuth, withCredentials }),
        [baseUrl, getAuth, withCredentials]
    );
    ...
    React.useEffect(() => {
        ensureRoot()
            .then((root) => loadChildren(root.id))
            .catch((e: unknown) => {
                if (e instanceof Error && e.name !== "AbortError") {
                    setError(e.message);
                }
            });
    }, [ensureRoot, loadChildren]);
    ...
    React.useEffect(() => {
        ...
        (async () => {
            ...
            setBusy(true);
            try {
                const res = await api<OrgNode[]>(
                    `/search?q=${encodeURIComponent(String(dq))}&limit=20`
                );
    ...
    const jumpTo = async (id: number) => {
        try {
            setBusy(true);
            const anc = await api<OrgNode[]>(`/${id}/ancestors?showDeleted=true`);
            await actions.expandPath(anc.map((a) => a.id));
            const node =
                state.nodes.get(id) || (await api<OrgNode>(`/${id}?showDeleted=true`));
    ...
    const selectFresh = async (n: OrgNode) => {
        try {
            setBusy(true);
            const fresh = await api<OrgNode>(`/${n.id}?showDeleted=true`);
            actions.setNode(fresh);
            setSelected(fresh);
        } catch {
            setSelected(n);
        } finally {
    ...
    const handleMove = (dragId: number, dropId: number) =>
        handleAction(async () => {
    ...
    const handlePermanentDelete = (id: number) => {
        if (
            !window.confirm(
                "DIQQAT! Bu tugun va uning BARCHA bolalari butunlay o'chib ketadi. Davom etasizmi?"
            )
        )
            return;
    ...
    const textBtnCn =
        "px-2 py-1 bg-gray-50 border border-gray-200 rounded-md text-xs cursor-pointer hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed";

    const selDeleted = selected
        ? isTrueFlag(selected.deleted) ||
        isTrueFlag(state.nodes.get(selected.id)?.deleted)
        : false;
            ...
            <div className="left-pane flex flex-col gap-2">
                <div className={baseCardCn}>
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            placeholder="Qidiruv: code yoki nom"
                            className="w-full border border-gray-200 rounded-md px-2.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                        {busy && <Spinner />}
                    </div>
                </div>
                        ...
                        <ul className="max-h-60 overflow-auto -mx-3 -my-1 list-none">
                            {hits.map((h) => (
                                <li key={h.id} className="border-t border-gray-100">
                                    <button
                                        onClick={() => jumpTo(h.id)}
                                        className={`bg-transparent border-none cursor-pointer text-left w-full px-3 py-1.5 hover:bg-gray-50 ${
                                            isTrueFlag(h.deleted) ? "text-red-700" : ""
                                        }`}
                                    >
                                    <span
                                        className={`${tagPillCn} ${
                                            isTrueFlag(h.deleted) ? "bg-red-100 text-red-700" : ""
                                        }`}
                                    >
                                      {h.code}
                                    </span>
                            ...
                            {hits.length === 0 && (
                                <li className="text-xs text-gray-500 px-3 py-1.5">Hech narsa topilmadi</li>
                            )}
                        </ul>
                    </div>
                )}
                ...
                <div className={baseCardCn}>
                    <div className="flex justify-between items-center mb-1.5">
                        ...
                        <div className="flex gap-1.5">
                            <button
                                ...
                                disabled={busy}
                            >
                                {showDeleted ? "O'chirilganlarni yashirish" : "O'chirilganlarni ko'rsatish"}
                            </button>
                            <button
                                ...
                                disabled={busy}
                            >
                                {showInactive ? "Noaktivlarni yashirish" : "Noaktivlarni ko'rsatish"}
                            </button>
                            <button
                                onClick={() => (busy ? undefined : actions.expandAll())}
                                className={textBtnCn}
                                disabled={busy}
                            >
                                Barchasini ochish
                            </button>
                            <button onClick={actions.collapseAll} className={textBtnCn} disabled={busy}>
                                Barchasini yopish
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="text-xs text-gray-500 mt-2">
                        Drag & drop: tugunni boshqa tugun ustiga tashlab re-parent qiling.
                    </div>
                </div>
            </div>
                ...
                <div className={baseCardCn}>
                    ...
                    {!selected ? (
                        ...
                        <dl className="text-sm grid grid-cols-[80px_1fr] gap-y-1.5 items-center">
                            ...
                            <dd className="text-gray-800">{selected.id}</dd>

                            <dt className="text-gray-500 font-medium">Code</dt>
                            <dd className="text-gray-800">{selected.code}</dd>
                            ...
                            <dd className="text-gray-800">{selected.level}</dd>

                            <dt className="text-gray-500 font-medium">Status</dt>
                            <dd className="text-gray-800">{selected.isActive ? "Aktiv" : "Noaktiv"}</dd>

                            <dt className="text-gray-500 font-medium">O'chirilgan</dt>
                            <dd className="text-gray-800">{selDeleted ? "Ha" : "Yo'q"}</dd>
